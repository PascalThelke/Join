...
async function filterContactsForAddTask() {
  document.getElementById("add-task-contact").innerHTML = "";
  let value = document
    .getElementById("add-task-assignet-to")
    .value.toLowerCase();
  for (let i = 0; i < contactList.length; i++) {
...
function focusSubtask(i) {
  ...
  document
    .getElementById("body")
    .setAttribute("onclick", `closeFunction(); startOnClickOutside(${i})`);
  document.getElementById(`subtask-edit-buttons${i}`).innerHTML =
    focusSubtaskHTML(i);
...
function editSubtask(i) {
  ...
  subtasksArray.splice(
    i,
    1,
    document.getElementById(`single-subtask-txt${i}`).innerHTML
  );
  document.getElementById(`subtask-edit-buttons${i}`).innerHTML =
    editSubtaskHTML(i);
  document
